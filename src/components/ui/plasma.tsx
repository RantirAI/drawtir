...
void main() {
  ...
  plasma += sin((uv.y * 10.0 + time) / 2.0);
  plasma += sin((uv.x * 10.0 + uv.y * 10.0 + time) / 2.0);
  plasma += sin(sqrt(uv.x * uv.x + uv.y * uv.y) * 10.0 + time);
  plasma /= 4.0;
  
  // Color cycling
  vec3 color = vec3(
    0.5 + 0.5 * sin(plasma * u_colorTint.x + time),
    0.5 + 0.5 * sin(plasma * u_colorTint.y + time + 2.0),
    0.5 + 0.5 * sin(plasma * u_colorTint.z + time + 4.0)
  );
...
interface PlasmaProps extends React.HTMLAttributes<HTMLDivElement> {
  speed?: number;
